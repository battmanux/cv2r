...
```{r}
hist(my_table[layer==1, value])
```

Then change pixels and revert to image

```{r}
my_table[layer==1 & y > 10, value := value + y ]
as.image(my_table)
```
...
```{r}
my_space <- dcast(my_table,  x + y ~ layer )

pairs(my_space[sample.int(size = 100, n =  nrow(my_space)),.(R,G,B, A)])
```
...
my_space[R < 100 & G > 50, R:=200]

as.image(melt(my_space, id.vars = c("x","y"), variable.name = "layer"))
